...
Tracking::Tracking(System *pSys, Map *pMap, Viewer* pViewer, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mpSystem(pSys), mpMap(pMap), mpViewer(pViewer)
{
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, cv::Mat &imD, const cv::Mat &imFlow, const cv::Mat &flowV,
                                const cv::Mat &maskSEM, cv::Mat &imDynaSem, const cv::Mat &mTcw_gt, const vector<vector<float> > &vObjPose_gt,
                                const double &timestamp, const bool bIsEnd)
{
    ...
    if(mState!=NO_IMAGES_YET)
    {
        cout << "data association, Update Current Frame From Last....." << endl;
    ...
    else 
    {
        ...
        for (int i = 0; i < vObjPose_gt.size(); ++i){
            // label
            mCurrentFrame.nSemPosi_gt[i] = vObjPose_gt[i][0];
        }
    }
    ...
    if(timestamp!=0 && bFrame2Frame == true)
    {
        ...
        for (int i = 0; i < TemperalMatch_subset.size(); i=i+1)
        {
            ...
            // cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,0,1), cv::DrawMatchesFlags::DEFAULT); // red

            cv::Point2f pt1,pt2;
            cv::Point2f point;
            const float r = 2;
            point = KeyPoints_tmp[0].pt;
            pt1.x = KeyPoints_tmp[0].pt.x-r;
            pt1.y = KeyPoints_tmp[0].pt.y-r;
            pt2.x = KeyPoints_tmp[0].pt.x+r;
            pt2.y = KeyPoints_tmp[0].pt.y+r;

            cv::rectangle(imRGB, pt1, pt2, cv::Scalar(0,255,0));
            cv::circle(imRGB, point, 2, cv::Scalar(0,255,0),-1);
        }

        std::cout << "total: " << TemperalMatch_subset.size() << ", cnt1: " << cnt1 << ", cnt2: " << cnt2 << ", cnt3: " << cnt3 << std::endl;
        // static and dynamic objects
        int invalid_points_1 = 0;
        int invalid_points_2 = 0;
        for (int i = 0; i < mCurrentFrame.vObjLabel.size(); ++i)
        {
            ...
            size_t h = l * 6364136223846793005u + 1442695040888963407;
            ...
            cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, standardColor, cv::DrawMatchesFlags::DEFAULT);
        }
        ...
        */
        ...
        auto UniObj = mCurrentFrame.vObjLabel;
        std::sort(UniObj.begin(), UniObj.end());
        UniObj.erase(std::unique( UniObj.begin(), UniObj.end() ), UniObj.end() );

        cout << "Unique object id: ";
        for (int i = 0; i < UniObj.size(); ++i)
            cout  << UniObj[i] << " ";
        cout << endl;

        //std::cout << "total object points=" << mCurrentFrame.vObjLabel.size() << ", invalid_points_1=" << invalid_points_1 << ", invalid_points_2=" << invalid_points_2 << std::endl;
    }
    ...
    if(timestamp!=0 && bFrame2Frame == true && mTestData==KITTI)
    {
        ...
        for (int i = 0; i < mCurrentFrame.vObjBoxID.size(); ++i)
        {
            if (!mTcw_gt.empty())
            {
                ...
                cv::rectangle(imDynaSem, pt1, pt2, cv::Scalar(0, 255, 0),2);
                ...
                float sp_est = mCurrentFrame.vSpeed[i].x/36;
                std::stringstream ss;
                ss << std::fixed << std::setprecision(1) << sp_est;  // 设置小数点位数

                std::string sp_est_string = ss.str();  // 将结果转为string

                // sp_gt.resize(5);
                
                // string output_gt = "GT:" + sp_gt + "km/h";
                string output_est = sp_est_string + "km/h";

                cv::putText(imDynaSem, output_est, cv::Point(pt1.x, pt1.y-10), cv::FONT_HERSHEY_DUPLEX, 0.6, CV_RGB(0,255,0), 2); // CV_RGB(255,140,0)
                // cv::putText(imDynaSem, output_gt, cv::Point(pt1.x, pt1.y-32), cv::FONT_HERSHEY_DUPLEX, 0.7, CV_RGB(255, 0, 0), 2);
            }
            else 
            {
                cv::Point pt1(vObjPose_gt[mCurrentFrame.vObjBoxID[i]][1], vObjPose_gt[mCurrentFrame.vObjBoxID[i]][2]);
                cv::Point pt2(vObjPose_gt[mCurrentFrame.vObjBoxID[i]][3], vObjPose_gt[mCurrentFrame.vObjBoxID[i]][4]);
                // cout << pt1.x << " " << pt1.y << " " << pt2.x << " " << pt2.y << endl;
                cv::rectangle(imDynaSem, pt1, pt2, cv::Scalar(0, 255, 0),2);
                ...
                // string output_gt = "GT:" + sp_gt + "km/h";
                string output_est = sp_est_string + "km/h";
                cv::putText(imDynaSem, output_est, cv::Point(pt1.x, pt1.y-10), cv::FONT_HERSHEY_DUPLEX, 0.8, CV_RGB(0,255,0), 1); // CV_RGB(255,140,0)
    ...
    if(timestamp!=0 && bFrame2Frame == true && mTestData==OMD && mpMap->vmCameraPose_GT.size() > 0)
    {
    ...
    if(timestamp!=0 && bFrame2Frame == true)
        mpViewer->publishPointPoseFrame(mpMap, imRGB, imDynaSem, flowV);
...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        // 计算当前帧下的静态3d点和动态3d点、初始化pose，保存到地图
    ...
    else
    {
        ...
        double cam_pos_time;
        s_1_1 = clock();
        // Get initial estimate using P3P plus RanSac for camera using static points
        ...
        std::cout << "TemperalMatch: " << TemperalMatch.size() << ", TemperalMatch_subset: " << TemperalMatch_subset.size() << std::endl;
        // Update motion model
        ...
        if (!mCurrentFrame.mTcw_gt.empty())
        {
        ...
        if (!mCurrentFrame.mTcw_gt.empty())
        {
            ...
            Last_Twc_gt = Converter::toInvMatrix(mLastFrame.mTcw_gt);
            Curr_Twc_gt = Converter::toInvMatrix(mCurrentFrame.mTcw_gt);
        }
        else 
        {
            mCurrentFrame.vSpeed.resize(ObjIdNew.size());
        ...
        for (int i = 0; i < ObjIdNew.size(); ++i)
        {
            ...
            if (!mCurrentFrame.mTcw_gt.empty())
            {
                ...
                L_w_p_inv = Converter::toInvMatrix(L_w_p);
                L_w_c_inv = Converter::toInvMatrix(L_w_c);
            ...
            if (ObjIdTest_in.size()<25)
            {
                cout << "Object Initialization Fail! ! !, valid " << ObjIdTest_in.size() << " 2d points in camera" << endl;
                mCurrentFrame.bObjStat[i] = false;
                mCurrentFrame.vObjMod[i] = cv::Mat::eye(4,4, CV_32F);
                mCurrentFrame.vObjCentre3D[i] = (cv::Mat_<float>(3,1) << 0.f, 0.f, 0.f);
            ...
            else 
            {
                ...
                cout << "estimated object speed: " << sp_est_norm << "km/h " << endl;
    ...
    bGlobalBatch = true;
    if (mbIsEnd) // bFrame2Frame f_id>=2
    {
        if (mpMap->vmCameraPose_GT.size() > 0)
        {
        ...
        if (bGlobalBatch && mTestData==KITTI)
        {
            ...
            std::cout << "end of FullBatchOptimization" << std::endl;
        }
    }
...
std::vector<std::vector<int> > Tracking::DynObjTracking()
{
    ...
    for (int i = 0; i < ObjId.size(); ++i)
    {
        ...
        if (sf_count/ObjId[i].size()>fSFDsThres && false)
        {
            // label this object as static background
            std::cout << "label this object as static background" << std::endl;
            for (int k = 0; k < ObjId[i].size(); ++k)
                mCurrentFrame.vObjLabel[ObjId[i][k]] = 0;
            continue;
        }
        else if (obj_center_depth/ObjId[i].size()>mThDepthObj || ObjId[i].size()<20)
        {
            obj_dis_tres[i]=-1;
            cout << "object " << sem_posi[i] <<" is too far away or too small! " << obj_center_depth/ObjId[i].size() << ", having " << ObjId[i].size() << " points" << endl;
...
cv::Mat Tracking::GetInitModelCam(const std::vector<int> &MatchId, std::vector<int> &MatchId_sub)
{
    ...
    // solve
    int iter_num = 500;
    double reprojectionError = 1.0, confidence = 0.98; // 0.5 0.3 0.4
    // double reprojectionError = 5.991, confidence = 0.99; // 0.5 0.3
    ...
    cout << "total: " << MatchId.size() << " Inlier Compare: " << "(1)AP3P RANSAC: " << inliers.rows << " (2)Motion Model: " << MM_inlier.size() << endl;
...
cv::Mat Tracking::GetInitModelObj(const std::vector<int> &ObjId, std::vector<int> &ObjId_sub, const int objid)
{
    ...
    // solve
    int iter_num = 500;
    double reprojectionError = 1.0, confidence = 0.98; // 0.3 0.5 0.4
    // double reprojectionError = 5.991, confidence = 0.99; // 0.5 0.3 
    cv::solvePnPRansac(pre_3d, cur_2d, camera_mat, distCoeffs, Rvec, Tvec, false,
               iter_num, reprojectionError, confidence, inliers, cv::SOLVEPNP_AP3P); // AP3P EPNP P3P ITERATIVE DLS
    ...
    if (PreObjID!=-1)
    {
        // calculate the re-projection error
        std::vector<int> MM_inlier;
        // VObjMod = T_w_p1 * T_p1_p2 * T_p1_w, T_p1_w(object in last frame time to world), T_p1_p2(object in current frame time to object in last frame time)
        ...
        for (int j = 0; j < TemporalMatch[i].size(); ++j)
        {
            ...
            if(i==0)
            {
                ...
                if (TemporalMatch[i][j]!=-1)
                {
                    ...
                    TraLet[0] = std::make_pair(i,TemporalMatch[i][j]); // the last one
                    TraLet[1] = std::make_pair(i+1,j); // the current one
...
void Tracking::RenewFrameInfo(const std::vector<int> &TM_sta)
{
    ...
    int tot_num = mvKeysTmp.size(), start_id = 0, step = 1;
